    ...
    if request.user.is_authenticated:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                ...
                messages.info(request, f"ProdukItem sudah diupdate menjadi: {order_produk_item.quantity}")
...
def remove_from_cart(request, slug):
    if request.user.is_authenticated:
        ...
        if order_query.exists():
            order = order_query[0]
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                order_produk_item = order.produk_items.get(produk_item__slug=produk_item.slug)
                if order_produk_item.quantity > 1:
                    order_produk_item.quantity -= 1
                    order_produk_item.save()
                    messages.info(request, "Quantity ProdukItem telah dikurangi")
                else:
                    order.produk_items.remove(order_produk_item)
                    messages.info(request, 'ProdukItem telah dihapus')
                return redirect('toko:produk-detail', slug=slug)
            else:
...
class HomeListView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if category:
            ...
            print("in here")  # Apply filtering by category if provided
        return queryset  # Remove the invalid non-printable character (U+00A0) here
    
def contact(request):
    return render(request, 'contact.html')

def detail(request, pk):
    item = get_object_or_404(ProdukItem, pk=pk)
    related_items = ProdukItem.objects.filter(kategori=item.category).exclude(pk=pk)[0:3]
