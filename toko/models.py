...
class ProdukItem(models.Model):
    ...
    harga = models.DecimalField(max_digits=8, decimal_places=3, default=Decimal('6.00'))
    harga_diskon = models.DecimalField(max_digits=8, decimal_places=3,blank=True, null=True)
    ...
    def discount_percentage(self):
        if self.harga_diskon:
            return round(((self.harga - self.harga_diskon) / self.harga) * 100)
        else:
            return 0
...
def produk_detail(request, slug):
    ...
    # Mendapatkan produk terkait berdasarkan kategori produk yang sedang dibuka
    related_items = ProdukItem.objects.filter(kategori=produk.kategori).exclude(slug=produk.slug)[:3]
    ...
    return render(request, 'toko/produk_detail.html', context)
